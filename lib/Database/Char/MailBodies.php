 ...
 */
class MailBodies extends AbstractChar
{
    /**
     * Constructor
     *
    ...
    protected function getIds()
    {
        ...
        $sql .= ' from ';
        $sql .= '`' . YAPEAL_TABLE_PREFIX . 'charMailMessages` as mm';
        $sql .= ' left join `' . YAPEAL_TABLE_PREFIX . 'charMailBodies` as mb';
        $sql .= ' on (mm.`ownerID` = mb.`ownerID` && mm.`messageID` = mb.`messageID`)';
        $sql .= ' where';
        $sql .= ' mb.`messageID` is null';
        $sql .= ' and mm.`ownerID` = ' . $this->ownerID;
        try {
                    ...
                    case \XMLReader::ELEMENT:
                        switch ($this->reader->localName) {
                            case 'row':
                                $row = array();
                                $row['messageID'] =
                                    $this->reader->getAttribute('messageID');
                                $row['body'] = $this->reader->readString();
                                $qb->addRow($row);
                                break;
                        }
                        break;
