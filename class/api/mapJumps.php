 ...
 * @subpackage Api_map
 */
class mapJumps implements IFetchApiTable, IStoreApiTable {
  /**
   ...
   */
  private $types = array('shipJumps' => 'I', 'solarSystemID' => 'I');
  /**
  ...
  public function __construct(array $params = array('serverName' =>'Tranquility')) {
    $this->tablePrefix = YAPEAL_TABLE_PREFIX . 'map';
    if (isset($params['serverName']) && is_string($params['serverName'])) {
      ...
      $mess .= ' for ' . $this->api . ' from map section in ' . __FILE__;
  ...
  function apiFetch() {
    ...
    try {
      ...
      $mess .= ' from map section in ' . __FILE__;
      $tracing->activeTrace(YAPEAL_TRACE_MAP, 2) &&
      $tracing->logTrace(YAPEAL_TRACE_MAP, $mess);
      $xml = YapealApiRequests::getCachedXml($cacheName, YAPEAL_API_MAP);
      if (empty($xml)) {
        $mess = 'getAPIinfo for ' . $this->api;
        $mess .= ' from map section in ' . __FILE__;
        $tracing->activeTrace(YAPEAL_TRACE_MAP, 2) &&
        $tracing->logTrace(YAPEAL_TRACE_MAP, $mess);
        $xml = YapealApiRequests::getAPIinfo($this->api, YAPEAL_API_MAP);
        if ($xml instanceof SimpleXMLElement) {
          // Store XML in local cache.
          YapealApiRequests::cacheXml($xml->asXML(), $cacheName, YAPEAL_API_MAP);
        };// if $xml ...
      };// if empty $xml ...
      if (!empty($xml)) {
        $this->xml = $xml;
  ...
  function apiStore() {
    ...
    if ($this->xml instanceof SimpleXMLElement) {
      $mess = 'Xpath for ' . $tableName . ' from map section in ' . __FILE__;
      $tracing->activeTrace(YAPEAL_TRACE_MAP, 2) &&
      $tracing->logTrace(YAPEAL_TRACE_MAP, $mess);
