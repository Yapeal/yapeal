...
if (conRev($ini_yapeal['version'])<=471) {
  ...
  $ini_yapeal['Database']['table_prefix'] = 'yapeal';
  $conf['accountData'] = $ini_yapeal['Database-account']['active'];
  $conf['charData'] = $ini_yapeal['Database-char']['active'];
  $conf['corpData'] = $ini_yapeal['Database-corp']['active'];
  $conf['eveData'] = $ini_yapeal['Database-eve']['active'];
  $conf['mapData'] = $ini_yapeal['Database-map']['active'];
	$ini_yapeal['Logging']['trace_active'] = $ini_yapeal['Tracing']['trace'];
  $passonlychange = "";
} else {
...
// Output Update site
OpenSite(SETUP,true);
echo '<form name="go_no_go" action="'.$_SERVER['SCRIPT_NAME'].'?lang='.$_GET['lang'].'&amp;edit=uselect" method="post">' . PHP_EOL
    ...
    .'  </tr>' . PHP_EOL
    .'</table>' . PHP_EOL
    .'<input type="hidden" name="config[db_action]" value="1" />' . PHP_EOL
    .'<br />' . PHP_EOL
    ...
    .'    <tr>' . PHP_EOL
    .'      <td class="tableinfolbl">'.PASSWORD.':</td>' . PHP_EOL
    .'      <td><input type="password" id="config_pass" name="config[config_pass]" value="" /> '.$passonlychange.'</td>' . PHP_EOL
    .'    </tr>' . PHP_EOL
    .'</table><br />' . PHP_EOL
