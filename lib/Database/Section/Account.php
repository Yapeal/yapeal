 ...
 */
class Account extends AbstractSection
{
    /**
     * Constructor
     *
    ...
    public function pullXML()
    {
        ...
        try {
            ...
            foreach ($keyList as $ky) {
                ...
                $apis =
                    $this->accessMask->maskToAPIs($ky['mask'], $this->section);
                if ($apis === false) {
                ...
                foreach ($apis as $api) {
                    ...
                    if (CachedUntil::cacheExpired($api, $ky['keyID']) === false
                    ) {
                        continue;
                    }
                    ++$apiCount;
    ...
    {
        $sql =
            'select urk.`keyID`,urk.`vCode`,urk.`activeAPIMask` as "RKMask",';
