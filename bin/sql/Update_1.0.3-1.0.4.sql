...
ALTER TABLE `{database}`.`{table_prefix}accountCharacters` ADD COLUMN `factionName` CHAR(24)
COLLATE utf8_unicode_ci DEFAULT '';
ALTER TABLE `{database}`.`{table_prefix}charMailMessages` ADD COLUMN `senderName` CHAR(50)
