...
TRUNCATE TABLE `TempConfig`;

-- -----------------------------------------------------
-- Data `TempConfig`
-- -----------------------------------------------------
INSERT INTO `TempConfig` (`Name`,`Value`)
SELECT C.`Name`,C.`Value`
FROM `%prefix%utilconfig` C
ON DUPLICATE KEY UPDATE `Value`=VALUES(`Value`);

-- -----------------------------------------------------
-- Drop Table `utilconfig`
-- -----------------------------------------------------
...
DEFAULT CHARSET=utf8
COLLATE=utf8_unicode_ci;
TRUNCATE TABLE `%prefix%utilConfig`;

-- -----------------------------------------------------
-- Data `utilConfig`
-- -----------------------------------------------------
INSERT INTO `%prefix%utilConfig` (`Name`,`Value`)
SELECT C.`Name`,C.`Value`
FROM `TempConfig` C
ON DUPLICATE KEY UPDATE `Value`=VALUES(`Value`);
