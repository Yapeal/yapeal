...
-- Data `corpAssetList`
-- -----------------------------------------------------
INSERT INTO `%prefix%corpAssetList` (`flag`,`itemID`,`lft`,
...
FROM `AssetList` A,`RegisteredCorporation` R
WHERE A.`ownerID` = R.`corporationID`
ON DUPLICATE KEY UPDATE `flag`=VALUES(`flag`),`lft`=VALUES(`lft`),
  `locationID`=VALUES(`locationID`),`ownerID`=VALUES(`ownerID`),`quantity`=VALUES(`quantity`),
  `rgt`=VALUES(`rgt`),`singleton`=VALUES(`singleton`),`typeID`=VALUES(`typeID`);

-- -----------------------------------------------------
-- Data `corpCorporationSheet`
-- -----------------------------------------------------
-- There is no data to move.
-- -----------------------------------------------------

-- -----------------------------------------------------
-- Data `corpContainerLog`
-- -----------------------------------------------------
-- There is no data to move.
-- -----------------------------------------------------
...
-- -----------------------------------------------------
-- Data `corpIndustryJobs`
-- -----------------------------------------------------
INSERT INTO `%prefix%corpIndustryJobs` (`activityID`,`assemblyLineID`,
...
FROM `IndustryJobs` I,`RegisteredCorporation` R
WHERE I.`ownerID` = R.`corporationID`
...
-- -----------------------------------------------------
-- Data `corpMarketOrders`
-- -----------------------------------------------------
INSERT INTO `%prefix%corpMarketOrders` (`accountKey`,`bid`,`changed`,`charID`,`duration`,
...
FROM `MarketOrders` M,`RegisteredCorporation` R
WHERE M.`ownerID` = R.`corporationID`
...
-- -----------------------------------------------------
-- Data `corpMemberTracking`
-- -----------------------------------------------------
-- There is no data to move.
-- -----------------------------------------------------

-- -----------------------------------------------------
-- Data `corpStarbaseList`
-- -----------------------------------------------------
-- There is no data to move.
-- -----------------------------------------------------
...
-- -----------------------------------------------------
-- Data `corpWalletJournal`
-- -----------------------------------------------------
INSERT INTO `%prefix%corpWalletJournal` (`accountKey`,`amount`,`argID1`,`argName1`,
...
SELECT W.`accountKey`,W.`amount`,
  ...
  W.`refTypeID`
FROM `WalletJournal` W,`RegisteredCorporation` R
WHERE W.`ownerID` = R.`corporationID`
...
-- -----------------------------------------------------
-- Data `corpWalletTransactions`
-- -----------------------------------------------------
INSERT INTO `%prefix%corpWalletTransactions` (`accountKey`,`characterID`,`characterName`,
...
SELECT W.`accountKey`,W.`characterID`,
  ...
  W.`typeName`
FROM `WalletTransactions` W,`RegisteredCorporation` R
WHERE W.`ownerID` = R.`corporationID`
