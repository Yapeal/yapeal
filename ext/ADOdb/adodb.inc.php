...
*/
	...
	*/
	 ...
	 * Execute SQL
	 *
	 * @param string|string[] $sql SQL statement to execute, or possibly an array holding prepared statement ($sql[0] will hold sql text)
	...
	*/
	...
	function CacheGetAssoc($secs2cache, $sql=false, $inputarr=false,$force_array = false, $first2cols = false)
	{
         ...
         * for you.
         *
         * @param  string|string[] $sql      SQL statement
     ...
     class ADORecordSet implements IteratorAggregate
     {
         ...
         */
         ...
         function UserDate($v, $fmt = 'Y-m-d')
         {
             ...
             // $tt == -1 if pre TIMESTAMP_FIRST_YEAR
             if (($tt === false || $tt == -1) && $v != false)
                 return $v; else if ($tt == 0)
                 return $this->emptyDate; else if ($tt
                 == -1
         ...
         function GetAssocKeys($upper = true)
         {
             ...
             for ($i = 0; $i < $this->_numOfFields; $i++) {
                 $o = $this->FetchField($i);
                 if ($upper === 2)
                     $this->bind[$o->name] = $i; else
                     $this->bind[($upper) ? strtoupper($o->name) :
                         strtolower($o->name)] = $i;
             }
         }
         /**
         ...
         */
         ...
         function FetchObject($isupper = true)
         {
             ...
             $i = 0;
             if (PHP_VERSION >= 5)
                 $o = clone($this->_obj); else $o = $this->_obj;
             for ($i = 0; $i < $this->_numOfFields; $i++) {
                 $name = $this->_names[$i];
                 if ($isupper)
                     $n = strtoupper($name); else $n = $name;
