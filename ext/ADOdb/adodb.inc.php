...
if (!defined('_ADODB_LAYER')) {
	...
	*/
    ...
    function ADODB_Setup()
    {
         ...
         */
        $ADODB_vers =
            'V5.14 8 Sept 2011  (c) 2000-2011 John Lim (jlim#natsoft.com). All rights reserved. Released BSD & LGPL.';
        /**
...
*/
    ...
    class ADOConnection
    {
        ...
        function outp_throw($msg, $src = 'WARN', $sql = '')
        {
            if (defined('ADODB_ERROR_HANDLER')
                && ADODB_ERROR_HANDLER == 'adodb_throw'
            ) {
                adodb_throw(
                    $this->databaseType,
                    $src,
         ...
         *
         * @returns mixed        The previous fetch mode
         */
	...
	*/
         ...
         * @param bool|array   $inputarr holds the input data to bind to. Null elements will be set to null.
         *
         * @return ADORecordSet_empty|ADORecordSet    RecordSet or false
         */
        ...
        function _Execute($sql, $inputarr = false)
        {
            ...
            if ($this->_queryID === true
                ...
                $rs = (class_exists($rsclass)) ? new $rsclass()
        ...
        function CreateSequence($seqname = 'adodbseq', $startID = 1)
        {
            ...
            return $this->Execute(
                sprintf($this->_genSeqSQL, $seqname, $startID)
            );
        }
        ...
        function ErrorMsg()
        {
            if ($this->_errorMsg) {
                return '!! ' . strtoupper(
                    $this->dataProvider . ' ' . $this->databaseType
                ) . ': ' . $this->_errorMsg;
            } else {
                return '';
            }
        ...
        function SelectLimit(
            ...
            if ($this->hasTop && $nrows > 0) {
                ...
                if ($ismssql) {
                    ...
                    $isaccess =
                        (strpos($this->databaseType, 'access') !== false);
                }
                if ($offset <= 0) {
        ...
        function &_rs2rs(&$rs, $nrows = -1, $offset = -1, $close = true)
        {
            ...
            if (!$dbtype) {
                $rs =
                    $rs; // required to prevent crashing in 4.2.1, but does not happen in 4.3.1 -- why ?
                return $rs;
            }
            ...
            $rs2->fetchMode = isset($rs->adodbFetchMode) ? $rs->adodbFetchMode
                : $rs->fetchMode;
            return $rs2;
        }
        /**
	...
	*/
        ...
        function _gencachename($sql, $createdir)
        {
            ...
            $m = md5(
                $sql . $this->databaseType . $this->database . $this->user
                . $mode
            );
	 ...
	 */
        ...
        function GetActiveRecords(
            ...
            $arr = $this->GetActiveRecordsClass(
                'ADODB_Active_Record',
                $table,
                $where,
                $bindarr,
        ...
        {
            $transaction_mode =
                $this->MetaTransaction($transaction_mode, $this->dataProvider);
...
*/
        ...
        function MetaColumns($table, $normalize = true)
        {
            ...
            if (!empty($this->metaColumnsSQL)) {
                ...
                $rs = $this->Execute(
                    sprintf(
                        $this->metaColumnsSQL,
                        ($normalize) ? strtoupper($table) : $table
                    )
                );
                if (isset($savem)) {
                    $this->SetFetchMode($savem);
         ...
         */
        function MetaColumnNames(
            $table,
            $numIndexes = false,
            $useattnum = false /* only for postgres */
        )
        {
        ...
        function PageExecute(
            ...
            if ($this->pageExecuteCountRows) {
                $rs = _adodb_pageexecute_all_rows(
                    $this,
                    $sql,
                    $nrows,
                    $page,
                ...
                $rs = _adodb_pageexecute_no_last_page(
                    $this,
                    $sql,
                    $nrows,
                    $page,
	...
	*/
        ...
        function PO_RecordCount($table = "", $condition = "")
        {
            ...
            if ($lnumrows == -1 && $this->connection) {
                IF ($table) {
                    ...
                    $resultrows = $this->connection->Execute(
            ...
            switch ($tmap) {
                ...
                case 'D':
                    if (!empty($this->connection)
                        && !empty($this->connection->datetime)
                    ) {
                        return 'T';
                    }
                    return 'D';
    ...
    class ADORecordSet_array extends ADORecordSet
    {
        ...
        function Fields($colname)
        {
            $mode = isset($this->adodbFetchMode) ? $this->adodbFetchMode
                : $this->fetchMode;
    ...
    function ADONewConnection($db = '')
    {
        ...
        $errorfn =
            (defined('ADODB_ERROR_HANDLER')) ? ADODB_ERROR_HANDLER : false;
        $false = false;
        if (($at = strpos($db, '://')) !== false) {
            ...
            if (strncmp($origdsn, 'pdo', 3) == 0) {
                ...
                if (sizeof($sch) > 1) {
                    ...
                    if ($sch[1] == 'sqlite') {
                        $dsna['host'] = rawurlencode(
                            $sch[1] . ':' . rawurldecode($dsna['host'])
                        );
                    } else {
                        $dsna['host'] = rawurlencode(
                            $sch[1] . ':host=' . rawurldecode($dsna['host'])
                        );
                    }
                    $dsna['scheme'] = 'pdo';
                }
            ...
            $dsna['host'] =
                isset($dsna['host']) ? rawurldecode($dsna['host']) : '';
            $dsna['user'] =
                isset($dsna['user']) ? rawurldecode($dsna['user']) : '';
            $dsna['pass'] =
                isset($dsna['pass']) ? rawurldecode($dsna['pass']) : '';
            $dsna['path'] =
                isset($dsna['path']) ? rawurldecode(substr($dsna['path'], 1))
                    : ''; # strip off initial /
            if (isset($dsna['query'])) {
