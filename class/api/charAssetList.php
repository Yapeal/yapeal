    ...
    if ($this->xml instanceof SimpleXMLElement) {
      if (count($this->xml) > 0) {
        try {
          ...
          self::shellSort($this->xml[1]['value'][0]['value']);
          $con = YapealDBConnection::connect(YAPEAL_DSN);
          $sql = 'delete from `' . $this->tableName . '`';
          $sql .= ' where `ownerID`=' . $this->characterID;
          // Clear out old tree for this owner.
          $con->Execute($sql);
          // Generate owner node as root for tree. It will need to be
          // updated after all the other records are upserted to correct 'rgt'.
          $row = array('flag' => '0', 'itemID' => $this->characterID,
            'lft' => '1', 'locationID' => '0', 'lvl' => '0',
            'ownerID' => $this->characterID, 'quantity' => '1', 'rgt' => '1',
      ...
      $mess = 'There was no XML data to store for ' . $this->tableName;
