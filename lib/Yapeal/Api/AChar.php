...
use Yapeal\Util\RegisteredCharacter;
use Yapeal\Util\RegisteredKey;
            ...
            switch ($e->getCode()) {
                ...
                case 212: // Authentication failure (final pass).
                    if (YAPEAL_REGISTERED_MODE != 'ignored') {
                        ...
                        $char =
                            new RegisteredCharacter($this->params['characterID']);
                        $char->isActive = 0;
                    ...
                    Logger::getLogger('yapeal')
                        ->warn($mess);
                    $key =
                        new RegisteredKey($this->params['keyID'], false);
                    $key->isActive = 0;
                    if (false === $key->store()) {
                        $mess =
                            'Could not deactivate keyID: '
                            . $this->params['keyID'];
                        Logger::getLogger('yapeal')
                            ->warn($mess);
                    }; // if $key->store() ...
                ...
                case 221: // Illegal page request! (Key accessMask outdated)
                    ...
                    if (YAPEAL_REGISTERED_MODE != 'ignored') {
                        $mess = 'Deactivating Eve API: ' . $this->api;
                        $mess .=
                            ' for characterID: ' . $this->params['characterID'];
